...
- **Break down tickets into specific Issues**
- **Create Job Cards from multiple issues**
- **Assign Job Cards to Mechanics (In-House) or Vendors (Outsourced)**
- **Manage Parts/Spares inventory for Job Cards**
...
tickets {
  ...
  -- Resolved: When ALL issues are marked "Done"
  -- Closed: When ALL issues are "Rated" by Supervisor
  ...
  -- Computed fields
  final_sla_end_date: date (Max of all linked issues' sla_end_date)
  overall_sla_status: enum (Adhered, Violated) -- Derived from issues
  
  created_by_user_id: uuid REFERENCES users
}

#### 1a. Issues (New Entity - The core work unit)

```sql
issues {
  id: uuid PRIMARY KEY
  issue_number: text (Auto-generated, e.g., T-1434-01)
  ticket_id: uuid REFERENCES tickets(id)
  
  -- Definiton
  description: text
  category: enum (Mechanical, Electrical, Body, Tyre, GPS, AdBlue)
  severity: enum (Minor, Major)
  work_type: enum (InHouse, Outsource)
  
  -- SLA (New, Per Issue)
  sla_days: integer
  sla_end_date: date
  sla_status: enum (Pending, Adhered, Violated)
  
  -- Execution
  job_card_id: uuid REFERENCES job_cards(id) -- Nullable until assigned
...
#### 1b. Job Cards (New Entity - Grouping Issues)

```sql
job_cards {
  id: uuid PRIMARY KEY
  job_card_number: text (Auto-generated unique ID)
  created_at: timestamp
  
  type: enum (In House, Outsource)
  -- Constraint: Cannot mix In House and Outsource issues in one card
  
  -- Assignment
  assigned_mechanic_id: uuid REFERENCES users (If In House)
  vendor_name: text (If Outsource)
  
  vehicle_number: text (Inherited from ticket)
  site: text
  
  status: enum (Open, Completed)
  -- Logic: Can be marked Completed only when ALL linked issues are "Done"
...
finance_entries {
  ...
  ticket_id: uuid REFERENCES tickets (optional, via job_card)
  site: text
...
- ✅ Edit ticket (Maintenance Exec only)
  - **Review Ticket**: Reject or Accept
  - **Create Issues**: Break down ticket into multiple issues (Electrical/Mechanical, Minor/Major, In-House/Outsource)
  - **Manage Job Cards**:
    - Select multiple issues to create a Job Card
    - Assign to Mechanic (In-House) or Vendor (Outsource)
    - **Constraint:** Cannot mix types in one Job Card
    - Add/Remove issues from Job Cards
  - **Job Card Detail View**:
    - Add parts/spares from inventory
    - Mark individual issues as DONE
    - Mark Job Card as COMPLETE (only when all issues done)
  - Update status logic (Pending -> Accepted -> WIP -> Resolved -> Closed)
  - Add comments
...
**1. SLA Engine (Re-targeted to Issues)**
- ✅ Auto-calculate `sla_end_date` on **ISSUE** creation
  - Based on issue severity + category matrix
- ✅ Ticket SLA = MAX(issue_sla_end_dates)
  - Ticket is "Violated" if ANY issue is removed/not completed by its SLA? Or if ticket isn't resolved by the LAST sla date?
  - **Decision**: Ticket SLA date is the *latest* due date among its issues.
- ✅ Assignment SLA tracking (Ticket level)
  - Flag if not assigned (Issues created) within 1 day
...
> [!NOTE]
> **Major Refactor Required**: Dashboards currently assume 1-ticket-1-SLA. Must be updated to visualize Issue-level performance.
> **Status**: Added to V0.3 Task List for redesign.
...
- ✅ Post-completion feedback
  - **Granular Rating**: Rate EACH issue individually (Good/Ok/Bad)
  - **Trigger**: Available once an issue is marked "Done"
  - **Closing Loop**: Ticket status -> "Closed" only when ALL issues are rated
...
- **Metrics Overview**: Add real-time counters to the dashboard cards.
    - **Tickets**: Total Open, SLA Warning Count.
    - **Issues**: Open Issues, High Severity Count.
    - **Job Cards**: Active Job Cards, Assigned to Vendor Count.
    - **SLA**: "About to be Violated" warning list.
- **Charts**: Visual breakdown of issues by category and vehicle.

### Advanced Features
- **Vendor Portal**: Limited access for vendors to update job cards directly.
- **Inventory Integration**: Link spare parts usage in Job Cards to an inventory system.
- **Mobile App**: Native mobile app for mechanics for offline capability.
