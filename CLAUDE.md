...
**The sync owns `vehicles.is_active` for feed-owned rows.** A vehicle is active exactly when the current Roorides feed still returns it — the same two-way rule `sites` gets, and rows are never deleted (`tickets.vehicle_number` and `job_cards.vehicle_number` are free text, so history survives regardless). Before MAIN-69 nothing ever revisited the flag, so a vehicle that left the feed kept `is_active = true` forever while `vehicle_sites` was rebuilt without it — leaving 69 rows in prod that no supervisor could see (their vehicle list is site-filtered) but every exec could still pick (a blank Site field means an unfiltered list).

Feed-owned means `raw_data IS NOT NULL`. **Vehicles added by hand through this page are deliberately exempt** — they have no feed entry to be absent from, so the rule would deactivate them on the next run. That carve-out is the one place vehicles can't simply copy the `sites` rule, because `sites` has no UI at all and this page does. MAIN-68 decides whether the manual add path should survive; if it goes, the exemption goes with it.
